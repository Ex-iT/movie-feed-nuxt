...
- **Caching**: Route rules in `nuxt.config.ts` set SWR values (`/` 15 min, `/rss` 30 min, `/api/v1/programmes` 15 min, `/api/v1/programmes/**` 30 min). `/_nuxt/**` and `/assets/**` get immutable cache headers. Server handlers use `defineCachedEventHandler` with matching `maxAge`/`staleMaxAge` values — no redundant `setHeader` calls. On Vercel, the edge cache controls freshness — there is no persistent disk cache between serverless invocations. Locally, Nitro persists cache to `.nuxt/cache/nitro/` which survives dev server restarts; clear it with `rm -rf .nuxt/cache/nitro` if stale data lingers.
